...
import PostType from "../apiRequests/PostType";
...
function TypeForm() {
	...
	useEffect(() => {
		if (params.typeId) {
			...
			const model = getRow(params.typeId);
			if (
				model === undefined ||
				model === null ||
				Object.keys(model) <= 0
	...
	async function saveClickHandler(event) {
		...
		if (isUpdate) {
			...
			types = {
				...
				idUserType: parseInt(params.typeId),
			};
		} else {
			setType("done");
			setTitle("successTitle");
			setContent("successContentAdd");
			types = inputFieldValuesObject;
		}

		response = await PostType(types).then((data) => {
			return data;
		});
	...
	return (
		<TemplateForm title={isUpdate ? t("updateType") : t("addTypes")}>
			<Grid container spacing={3}>
				...
				<Grid item xs={12}>
					<InputField
						...
						value={inputFieldValuesObject}
						valueHandler={handleChange}
						validationValues={validationMessageAndValidityObject["description"]}
						required={true}
					/>
				</Grid>
			</Grid>
