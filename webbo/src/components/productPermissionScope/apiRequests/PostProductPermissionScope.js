 ...
 */
async function PostProductPermissionScope(model) {
    const URL = process.env.REACT_APP_API_PARTNER + "/addProductPermissionScope";
    const data = await FetchRequest(URL, "post", model);

    return data;
}

export default PostProductPermissionScope;
