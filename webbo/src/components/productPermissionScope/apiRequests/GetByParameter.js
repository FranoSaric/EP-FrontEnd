...
async function GetByParameter({ filter, page, pageSize }) {
    const URL = process.env.REACT_APP_API_PARTNER + "/getProductPermisionScope";
    ...
    JSON.parse(data.data).forEach((element) => {
        dataArray.push({
            id: element.IDProductPermissionScope,
            productID: element.ProductID,
            permissionScopeID: element.PermissionScopeID,
            productName: element.ProductName,
            scopePermissionName: element.ScopeName
        });
    });
