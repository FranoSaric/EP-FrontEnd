...
function ChangePassword() {
	...
	//validation state
	const [validationMessageAndValidityObject, setValidationMessageAndValidityObject] =
		useState(
		...
		if (
			...
			if (response !== undefined) {
				if (response.status === 101) {
					...
					setFormIsValid(
						useInputFormValidation.validateWholeForm(temp)
					);
	...
	return (
		<TemplateForm title={t("changePassword")} size={"small"}>
			<Grid container spacing={3}>
				<Grid item xs={12} sm={6}>
					<InputField
						...
						validationValues={
							validationMessageAndValidityObject["oldPassword"]
						}
						required={true}
					/>
				</Grid>
			...
			<Grid container spacing={3}>
				<Grid item xs={12} sm={6}>
					<InputField
						...
						valueHandler={handleChange}
						validationValues={
							validationMessageAndValidityObject["newPassword"]
						}
						required={true}
					/>
				</Grid>
			...
			{ctx.isModalOn && (
				<MsgBox type={type} title={title} content={content} />
			)}
		</TemplateForm>
	);
}
